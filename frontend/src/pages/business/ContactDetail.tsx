...
export default function ContactDetail() {
  ...
  // Seed the editable fields once the card loads (and re-seed after a save/refetch),
  // without a setState-in-effect cascade: adjust during render, keyed on the
  // contact's identity + updatedAt so it only fires when the server data actually changes.
  const [syncedContact, setSyncedContact] = useState<string | null>(null);
  const contactVersion = card.data
    ? `${card.data.contact.id}:${card.data.contact.updatedAt}`
    : null;
  if (contactVersion && contactVersion !== syncedContact) {
    setSyncedContact(contactVersion);
